...
$clear='';
if($attachment_ids){
    $add_class='col-md-10 col-sm-12 col-xs-12';
    $clear='product-share-padding';
}
wp_enqueue_script('ovez-slick');

?>

<div class="images na-gallery-image product_vertical <?php echo esc_attr($clear);?> clearfix" data-number="<?php echo esc_attr($number_thumbnails);?>">

    <?php do_action( 'woocommerce_product_thumbnails' ); ?>

    <div class="na-product-image <?php echo esc_attr($add_class);?>">
        <div class="product-gallery-slider gallery-main">
            ...
            if ( has_post_thumbnail() ) {
                $html  = '<figure data-thumb="' . get_the_post_thumbnail_url( $post->ID, 'shop_thumbnail' ) . '" class="woocommerce-product-gallery__image first"><a  class="nmc" href="' . esc_url( $full_size_image[0] ) . '"  data-size="'.esc_attr($full_size_image[1]).'x'.esc_attr($full_size_image[2]).'">';
